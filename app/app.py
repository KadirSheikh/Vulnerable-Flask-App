...
def get_issues(): 
    try: 
        ...
        issues_list = [{'issue': issue.issue, 'rca': issue.rca, 'improvement': issue.improvement} for issue in issues] 
        print("Fetched issues successfully.") 
        return jsonify(issues_list), 200 
    except Exception as e: 
        print("Error fetching issues: ", str(e)) 
        return jsonify({'Error': 'Unable to fetch issues'}), 500 
...
def reg_customer(): 
    try: 
        ...
        print("Error in reg_customer: ", str(e)) 
...
def reg_user(): 
    try: 
        ...
        if content: 
            ...
            email = content['email'] 
            ccn = content['ccn'] 
            cust = Customer(first_name=first_name, last_name=last_name, email=email, username=username, password=password, ccn=ccn) 
            db.session.add(cust) 
            db.session.commit() 
            user_created = 'Customer: {0} has been created'.format(username) 
            return jsonify({'Created': user_created}),200 
    except Exception as e: 
        print("Error in reg_user: ", str(e)) 
        return jsonify({'Error': str(e)}),404 
